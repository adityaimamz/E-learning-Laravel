...
@section('container')
    ...
    <div class="ps-4 pe-4 mt-4 pt-4">
        <h2 class="display-6 fw-bold"><a href="{{ route('viewSurvey') }}"><button type="button"
                    ...
                    <i class="fa-solid fa-arrow-left"></i></button></a> Update Survey</h2>
    ...
    <div class="">
        <div class="row p-4">
            <h4 class="fw-bold text-primary"><i class="fa-solid fa-pen"></i> Update Data Survey</h4>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="user_id" aria-label="Default select example" required>
                                    @foreach ($users as $user)
                                        <option value="{{ $user->id }}" {{ $user->id == $survey->user_id ? 'selected' : '' }}>{{ $user->name }}</option>
                                    @endforeach
                                </select>
                                @error('guru')
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select" name="kelas_id" aria-label="Default select example" required>
                                    @foreach ($classes as $class)
                                        <option value="{{ $class->id }}" {{ $class->id == $survey->kelas_id ? 'selected' : '' }}>{{ $class->name }}</option>
                                    @endforeach
                                </select>
                                @error('kelas')
                                    <div class="text-danger small">
