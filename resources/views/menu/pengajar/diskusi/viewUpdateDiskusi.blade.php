    ...
    <div class="">
        <div class="row p-4">
            ...
            <div class="col-12 col-lg-12 bg-white rounded-2">
                <div class="mt-4">
                    <div class="p-4">
                        <form action="{{ route('updateDiskusi') }}" id="form-main" method="POST"
                            ...
                            <div class="mb-3 row">
                                ...
                                <div class="col-4 col-lg form-check form-switch">
                                    <input class="form-check-input" name="opened" type="checkbox" role="switch"
                                        id="opened" @if ($diskusi->isHidden == 0) checked @endif>
                                </div>
                            </div>
                            {{-- Nama Diskusi --}}
                            <div class="mb-3">
                                <label for="nama" class="form-label">Judul Diskusi</label>
                                <input type="hidden" name="kelasId" value="{{ encrypt($kelasId) }}" readonly>
                                <input type="hidden" name="diskusiId" value="{{ encrypt($diskusi['id']) }}" readonly>
                                <input type="hidden" name="mapelId" value="{{ $mapel['id'] }}" readonly>
                                <input type="text" class="form-control" id="nama" name="name"
                                    placeholder="Inputkan judul diskusi..." value="{{ old('name', $diskusi['name']) }}"
                                    required>
                                @error('name')
