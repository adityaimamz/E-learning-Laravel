...
class KelasMapelController extends Controller
{
    ...
    public function viewKelasMapel($x, $token, Request $request)
    {
        if ($token) {
            ...
            return view('menu.kelasMapel.viewKelasMapel', ['editor' => $editor, 'assignedKelas' => $assignedKelas, 'roles' => $roles, 'title' => 'Dashboard', 'kelasMapel' => $kelasMapel, 'ujian' => $ujian, 'materi' => $materi, 'mapel' => $mapel, 'kelas' => $kelas, 'tugas' => $tugas, 'rekomendasi' => $rekomendasi]);
        } else {
            abort(404);
        }
    }
